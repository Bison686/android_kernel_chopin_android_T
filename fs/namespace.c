...
retry:
	...
	res = ida_get_new_above(&susfs_mnt_id_ida, DEFAULT_SUS_MNT_ID, &mnt->mnt_id);
...
static int mnt_alloc_group_id(struct mount *mnt)
{
	...
	if (mnt->mnt_id >= DEFAULT_SUS_MNT_ID) {
		...
		if (!ida_pre_get(&susfs_mnt_group_ida, GFP_KERNEL))
			return -ENOMEM;
		res = ida_get_new_above(&susfs_mnt_group_ida, DEFAULT_SUS_MNT_GROUP_ID, &mnt->mnt_group_id);
